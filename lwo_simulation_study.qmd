...
We evaluate four treatment effect patterns: - a null scenario where there is no treatment effect, but there are covariate effects and main time effects (patients generally get better over time). - a scenario that mimics the SID trial, the almost null treatment effect is derived from SID trial data - a hypothetical positive SID trial where the treatment also interacts with time and therefore magnifies over time. - a constant treatment effect
...
```{=tex}
\begin{aligned}
 P(Y_j^{(t)} \succcurlyeq Y_i^{(t)}|A_j=1,A_i=0,\mathbf{X_j},\mathbf{X_i}) & =g^{-1}\Bigl(\beta_0+\beta_1 visit_1+\beta_2visit_2 \\
...
true_wos <- lapply(scenario_effects, function(pars) {
  calculate_adjusted_win_odds_multiple_visits(
    N_approx = 1e6,
    baseprobs = baseprobs,
...
for(sce in scenario_list[c(13,19)])
{
# try smaller sample size for the Mancle modification
  scenario <- sce$scenario
  # N <- sce$N
  N <- 10
  rho <- sce$rho
  corMatrix <- generate_corMatrix(n_visits = length(visits),
                                  rho = rho,
                                  corstr = corstr)
...
## plot of coverage for N=50, with modified sandwich estimator

```{r}
coverage_df_std <- coverage_df |>
  filter(N==50) |>
  mutate(Type = "Standard")
coverage_list <- sapply(scenario_list[c(1,4,7,10,13,16,19,22)], function(sce)
{
  scenario <- sce$scenario
  N <- sce$N
  rho <- sce$rho
  estimand <- true_wos[[scenario]]
  res <- assess_results(N=N,rho=rho,scenario = scenario,
                 estimand = estimand,type="modified")
  coverage <- res$`table of performance metrics`$coverage
  out <- data.frame(
    scenario = scenario,
    N = N,
...
coverage_df$rho_f <- factor(coverage_df$rho,
                             levels=c(0.3,0.6),
                             labels=c(TeX("$\\rho$ = 0.3"),TeX("$\\rho$ = 0.6")))
coverage_df$Type <- "Modified"
df <- rbind(coverage_df_std,coverage_df)
df |>
ggplot(aes(x=visits,y=coverage,color=Type))+
  facet_grid(rho_f~scenario,labeller = label_parsed)+
  geom_line()+
  geom_point()+
  labs(color="Sample size"
  ...
  theme_bw()+
  ylim(c(0.92,0.96))+
  scale_color_manual(values = c("#EA8379","#7DAEE0"))+
  geom_errorbar(aes(ymin=coverage-1.96*sd, ymax=coverage+1.96*sd), width=.2,
                 position=position_dodge(0.05))+
  geom_hline(yintercept = 0.95,linetype=2,color="coral")+
  labs(x="Visit (week)",y="Coverage probability")+
...
## Plot of coverage for N=10

```{r}
coverage_list <- sapply(scenario_list[c(1,7,13,19)], function(sce)
{
  scenario <- sce$scenario
  N <- 10
  rho <- sce$rho
  estimand <- true_wos[[scenario]]
  res <- assess_results(N=N,rho=rho,scenario = scenario,
                 estimand = estimand,type="comp")
  coverage_std <- res$`table of performance metrics`$coverage
  res <- assess_results(N=N,rho=rho,scenario = scenario,
                 estimand = estimand,type="modified")
  coverage_modified <- res$`table of performance metrics`$coverage
  out <- data.frame(
    scenario = scenario,
    N = N,
    rho = rho,
    visits=rep(c(0,4,8),2),
    coverage = c(coverage_std,coverage_modified),
    Type = rep(c("Standard","Modified"),each=3)
  )
  return(out)
},simplify = FALSE)
